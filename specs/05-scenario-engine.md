...
Scenarios are part of the full `AppState` and are persisted via two mechanisms — see Spec 04 §9 for the complete Save & Import specification. Summary:

- **Auto-save**: full state (including scenarios) written to `localStorage` on every change, debounced 500ms; restored silently on next app load
- **Manual save**: user clicks "Save" in the header to download a `.json` file to their computer
- **Import**: user clicks "Import" to load a previously saved `.json` file, replacing current state after confirmation
- Simulation results are never persisted — scenarios are re-run after import
...
## Changelog
- 2026-05-09T16:19:58Z: §6 Scenario Persistence updated — replaced standalone export/import description with reference to Spec 04 §9; auto-save and manual save/import are now fully specified there
